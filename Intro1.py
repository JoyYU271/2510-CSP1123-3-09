...
class dialog:
    ...
    def reset_typing(self):
        ...
        # Check if the current line (self.current_line_data) has text to type.
        if self.current_line_data:
            text_content = self.current_line_data.get("text", "")
            if not text_content: # If no text content, instantly complete typing
                self.typing_complete = True
                self.current_text_display = ""
        else: # If self.current_line_data is None (error state or end of list), instantly complete.
            self.typing_complete = True
            self.current_text_display = ""
    ...
    def handle_space(self, screen_surface=screen, keys=None):
        global game_chapter
    ...
    def draw(self, screen_surface):
        ...
        if self.choices_active: # THIS IS THE CRITICAL IF FOR CHOICES
            ...
            draw_text(screen, self.text_size, 40, (0, 0, 0), dialog_x + 200, dialog_y + 10, center=False)
    ...
    def fade(self, surface, cg_list=None, fade_in=True):

        fade_surface = pygame.Surface(surface.get_size()).convert_alpha()
        fade_surface.fill((0, 0, 0))


        cg_image_scaled = None
        if cg_list:
            cg_image_scaled = pygame.transform.scale(
                cg_list[0], surface.get_size()
            ).convert_alpha()


        alpha_iter = range(0, 256, 10) if fade_in else range(255, -1, -10)


        for alpha in alpha_iter:
            fade_surface.set_alpha(alpha if fade_in else alpha)   
            surface.fill((0, 0, 0))                               

            if cg_image_scaled:                                  
                surface.blit(cg_image_scaled, (0, 0))
...
class ObjectDialogue: 
    ...
    def draw(self, screen):
        ...
        # ✅ 如果正在显示 CG，就把它当作背景图先 blit 出来
        if hasattr(self, 'showing_cg') and self.showing_cg:
            cg_surface = self.get_current_cg()
            if cg_surface:
                cg_surface_scaled = pygame.transform.scale(cg_surface, screen.get_size())
                screen.blit(cg_surface_scaled, (0, 0))
...
class Rooms:    # class Level in tutorial
    ...
    def load_room(self, room_name, facing="left"):
        ...
        for npc_id, npc_info in npc_data.items(): 
            ...
            if not isinstance(pos, list) or len(pos) != 2:
                    print(f"ERROR: NPC '{name}' has invalid position: {pos}. Skipping.")
                    continue  # 跳过这个 NPC
         
            npc = NPC(pos[0], pos[1], name, dialogue_id=dialogue_id)
    ...
    def handle_dialogue_event(self, event_name, event_data=None):
        ...
        elif event_name == "back_to_reality":
            print("Handling back_to_reality: fade to Player_room and trigger back_reality dialogue")
            self.fading = True
            self.fade_alpha = 0
            self.next_room_after_transition = "Player_room"
            self.back_to_reality_flag = True  # 标志：fade 完之后触发 back_reality
            ...
            if isinstance(dialogue_instance_at_frame_start, dialog) and dialogue_instance_at_frame_start.choices_active:
                ...
                if keys[pygame.K_SPACE] and self.space_released:
                    ...
                    if isinstance(dialogue_instance_at_frame_start, ObjectDialogue):
                        ...
                        dialogue_instance_at_frame_start.handle_space(screen)
